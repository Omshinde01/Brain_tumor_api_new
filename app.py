    ...
    if uploaded_file and predict_clicked:
        ...
        if model is not None:
            ...
            with st.spinner("Generating AI advice..."):
                condition_name = predicted_class.replace('_', ' ')
                advice_text = ""

                # 1. ATTEMPT VETTED GOVT API (FREE)
                try:
                    import requests
                    # Search for the condition in the MyHealthfinder database
                    vetted_url = f"https://health.gov/myhealthfinder/api/v3/topicsearch.json?keyword={condition_name}"
                    v_res = requests.get(vetted_url, timeout=5)
                    v_data = v_res.json()

                    if v_data['Result']['Total'] != "0":
                        resource = v_data['Result']['Resources']['Resource'][0]
                        # Use the summary/content from the government database
                        advice_text = f"### Professional Overview: {condition_name.title()}\n\n"
                        advice_text += resource['Sections']['section'][0]['Description']
                        advice_text += "\n\n*Source: Health.gov MyHealthfinder*"
                except:
                    pass # Silently fail and move to Gemini fallback

                # 2. FALLBACK TO GEMINI (If no vetted info found)
                if not advice_text:
                    try:
                        prompt = f"""
                        A patient has been flagged for {condition_name} in a brain MRI scan.
                        Provide:
            ...
            # UI Component remains untouched
